...
class Student extends Admin_Controller
{
    ...
    public function create()
    {
        ...
        $this->form_validation->set_rules('category_id', $this->lang->line('courses'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('class_id', $this->lang->line('class'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('section_id', $this->lang->line('section'), 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($insert) {
                ...
                $data_student_login = array(
                    //diubah pada 12/11/2021
                    // 'username' => $this->student_login_prefix . $insert_id, $this->input->post('roll_no');
                ...
                $student_login_detail = array('id' => $insert_id, 'credential_for' => 'student', 'username' => $this->input->post('roll_no'), 'password' => $user_password, 'contact_no' => $this->input->post('mobileno'), 'email' => $this->input->post('email'));
                    ...
                    if (!empty($result)) {
                        ...
                        for ($i = 1; $i <= count($result); $i++) {
                            ...
                            if (!empty($insert_id)) {
                                ...
                                $student_login_detail = array('id' => $insert_id, 'credential_for' => 'student', 'username' => $this->input->post('roll_no'), 'password' => $user_password, 'contact_no' => $mobile_no, 'email' => $email);
                                $this->mailsmsconf->mailsms('login_credential', $student_login_detail);
    ...
    public function dtstudentlist()
    {
        ...
        if (!empty($students->data)) {
            foreach ($students->data as $student_key => $student) {
                ...
                $row[] = $this->customlib->dateformat($student->admission_date);
